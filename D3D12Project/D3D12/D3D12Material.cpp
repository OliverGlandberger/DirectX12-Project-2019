...
{	///////////////////////////////////////////
	...
	if (type == D3D12Material::ShaderType::VS)
	{
		entryPointString = "VS_main";
		shaderModelString = "vs_5_0";
	}
	else if (type == D3D12Material::ShaderType::PS)
	{
		entryPointString = "PS_main";
		shaderModelString = "ps_5_0";
	}
	else if (type == D3D12Material::ShaderType::GS)
	{
		entryPointString = "GS_main";
		shaderModelString = "gs_5_0";
	}
	else if (type == D3D12Material::ShaderType::CS)
	{
...
std::vector<std::string> D3D12Material::expandShaderText(std::string& shaderText, D3D12Material::ShaderType type)
{
...
D3D12Material& D3D12Material::addDefine(const std::string& defineText, ShaderType type)
{
...
void D3D12Material::setShader(const std::string& shaderFileName, D3D12Material::ShaderType type)
{
...
void D3D12Material::removeShader(D3D12Material::ShaderType type)
{
	unsigned int shaderType = this->shaderObjects[static_cast<unsigned int>(type)];

	this->shaderNames[shaderType] = "REMOVED";

	if (type == D3D12Material::ShaderType::VS && this->m_shaderDataBlob_VS != nullptr)
		this->m_shaderDataBlob_VS->Release();

	else if (type == D3D12Material::ShaderType::PS && this->m_shaderDataBlob_PS != nullptr)
...
void D3D12Material::addConstantBuffer(std::string name, unsigned int location)
{
	...
	this->constantBuffers[location] = DBG_NEW D3D12ConstantBuffer(name, location);
}

void D3D12Material::setConstantBuffer(const void* data, size_t size, unsigned int location)
{
	this->constantBuffers[location]->setInitData(data, size, location);
}

void D3D12Material::updateConstantBuffer(const void* data, size_t size, unsigned int location)
{
	this->constantBuffers[location]->setData(data, size, location);
}

int D3D12Material::enable()
{
	for (auto id : constantBuffers) {
		id.second->bind();
	}
	return 0;
}
